...
FAutoConsoleVariableRef CVarNetVisualizeRelevancyTestPoints(
    TEXT("p.NetVisualizeRelevancyTestPoints"), NetVisualizeRelevancyTestPoints, TEXT("") TEXT("0: Disable, 1: Enable"), ECVF_Cheat);

static int32 NetEnablePauseRelevancy = 1;
FAutoConsoleVariableRef CVarNetEnablePauseRelevancy(
    TEXT("p.NetEnablePauseRelevancy"), NetEnablePauseRelevancy, TEXT("") TEXT("0: Disable, 1: Enable"), ECVF_Cheat);
...
void AShooterCharacter::UpdatePawnMeshes()
{
    ...
    Mesh1P->VisibilityBasedAnimTickOption =
        !bFirstPerson ? EVisibilityBasedAnimTickOption::OnlyTickPoseWhenRendered : EVisibilityBasedAnimTickOption::AlwaysTickPoseAndRefreshBones;
...
bool AShooterCharacter::Die(float KillingDamage, FDamageEvent const& DamageEvent, AController* Killer, AActor* DamageCauser)
{
    ...
    UDamageType const* const DamageType =
        DamageEvent.DamageTypeClass ? DamageEvent.DamageTypeClass->GetDefaultObject<UDamageType>() : GetDefault<UDamageType>();
...
void AShooterCharacter::ReplicateHit(
    float Damage, struct FDamageEvent const& DamageEvent, class APawn* PawnInstigator, class AActor* DamageCauser, bool bKilled)
{
...
void AShooterCharacter::SetCurrentWeapon(AShooterWeapon* NewWeapon, AShooterWeapon* LastWeapon)
{
    ...
    if (NewWeapon)
    {
        NewWeapon->SetOwningPawn(this); // Make sure weapon's MyPawn is pointing back to us. During replication, we can't guarantee APawn::CurrentWeapon will
                                        // rep after AWeapon::MyPawn!
...
void AShooterCharacter::Tick(float DeltaSeconds)
{
    ...
    if (GEngine->UseSound())
    {
        if (LowHealthSound)
        {
            if ((this->Health > 0 && this->Health < this->GetMaxHealth() * LowHealthPercentage) &&
                (!LowHealthWarningPlayer || !LowHealthWarningPlayer->IsPlaying()))
            {
                LowHealthWarningPlayer = UGameplayStatics::SpawnSoundAttached(
                    LowHealthSound, GetRootComponent(), NAME_None, FVector(ForceInit), EAttachLocation::KeepRelativeOffset, true);
                if (LowHealthWarningPlayer)
                {
                    LowHealthWarningPlayer->SetVolumeMultiplier(0.0f);
                }
            }
            else if ((this->Health > this->GetMaxHealth() * LowHealthPercentage || this->Health < 0) && LowHealthWarningPlayer &&
    ...
    FAudioThread::RunCommandOnAudioThread(
        [UniqueID, bLocallyControlled]() { USoundNodeLocalPlayer::GetLocallyControlledActorCache().Add(UniqueID, bLocallyControlled); });
...
void AShooterCharacter::BeginDestroy()
{
    ...
    {
        const uint32 UniqueID = GetUniqueID();
        FAudioThread::RunCommandOnAudioThread([UniqueID]() { USoundNodeLocalPlayer::GetLocallyControlledActorCache().Remove(UniqueID); });
    }
}
...
void AShooterCharacter::SetJetpackActivation(bool Value)
{
    bJetPackActive = Value;
}

bool AShooterCharacter::IsJetpackActive() const
{
    return bJetPackActive;
}

void AShooterCharacter::FuelConsumption()
{
    if (FuelAmount == 0)
    {
        GetWorldTimerManager().ClearTimer(TimerHandle_JetpackFuelTimer);
    ...
    const auto FuelValue = FMath::Clamp((FuelAmount - FuelConsumptionAmount), 0.0f, 100.0f);
    SetFuelAmount(FuelValue);
}

void AShooterCharacter::FuelRecharge()
{
    if (FuelAmount == 100)
    {
        GetWorldTimerManager().ClearTimer(TimerHandle_JetpackRechargeTimer);
    }
    const auto FuelValue = FMath::Clamp((FuelAmount + FuelRechargeAmount), 0.0f, 100.0f);
    SetFuelAmount(FuelValue);
}
