...
void AShooterHUD::DrawWeaponHUD()
{
    ...
    if (MyWeapon)
    {
        ...
        // PRIMARY WEAPON
        {
            ...
            const float PriClipPosY =
                Canvas->ClipY - Canvas->OrgY - (PriWeapOffsetY + (PrimaryWeapBg.VL + MyWeapon->PrimaryClipIcon.VL) / 2 + Offset) * ScaleUI;
            ...
            MakeUV(PrimaryWeapBg, TileItem.UV0, TileItem.UV1, PrimaryWeapBg.U + PrimaryWeapBg.UL - RestWidth / ScaleUI, PrimaryWeapBg.V, RestWidth / ScaleUI,
                PrimaryWeapBg.VL);
            Canvas->DrawItem(TileItem);
            ...
            const float BottomTextPosX =
                Canvas->ClipX - Canvas->OrgX - (PriWeaponBoxWidth + Offset * 2 + (BoxWidth + SizeX * BottomTextScale) / 2.0f) * ScaleUI;
        ...
        if (SecondaryWeapon)
        {
            ...
            const float SecWeapPosY =
                Canvas->ClipY - Canvas->OrgY - (SecWeapOffsetY + (SecondaryWeapBg.VL + SecondaryWeapon->SecondaryIcon.VL) / 2.0f + Offset) * ScaleUI;
            ...
            const float SecClipPosY =
                Canvas->ClipY - Canvas->OrgY - (SecWeapOffsetY + (SecondaryWeapBg.VL + SecondaryWeapon->SecondaryClipIcon.VL) / 2.0f + Offset) * ScaleUI;
            ...
            MakeUV(SecondaryWeapBg, TileItem.UV0, TileItem.UV1, SecondaryWeapBg.U + SecondaryWeapBg.UL - RestWidth / ScaleUI, SecondaryWeapBg.V,
                RestWidth / ScaleUI, SecondaryWeapBg.VL);
            Canvas->DrawItem(TileItem);
            ...
            const float TopTextPosX =
                Canvas->ClipX - Canvas->OrgX - (SecWeaponBoxWidth + Offset * 2 + (SecClipBoxWidth + SizeX * TopTextScale) / 2.0f) * ScaleUI;
...
void AShooterHUD::DrawNVIDIAReflexTimers()
{
    TArray<ILatencyMarkerModule*> LatencyMarkerModules =
        IModularFeatures::Get().GetModularFeatureImplementations<ILatencyMarkerModule>(ILatencyMarkerModule::GetModularFeatureName());
    ...
    if (MyGameState && MyGameState->RemainingTime > 0)
    {
        ...
        else if (MyGameState->GetMatchState() == MatchState::InProgress)
        {
            ...
            TextItem.Position = FVector2D(
                TimerPosX + Offset * 1.5f * ScaleUI + TimerIcon.UL * ScaleUI, TimerPosY + (TimePlaceBg.VL * ScaleUI - SizeY * TextScale * ScaleUI) / 2);
        ...
        if (MyPC && MyGameState && MatchState == EShooterMatchState::Playing)
        {
            ...
            if (MyPlayerState)
            {
                if (MyGameState->NumTeams > 1) // team based game
                {
                    ...
                    {
                        if (MyGameState->TeamScores.Num() > MyTeam && MyGameState->TeamScores[MyTeam] >= MyGameState->TeamScores[i] && MyTeam != i)
                        {
                            MyPos--;
                        }
                    }
                ...
                Canvas->StrLen(BigFont, Text, SizeX, SizeY);
                Canvas->DrawIcon(PlaceIcon, Canvas->ClipX - Canvas->OrgX - BoxWidth - (SizeX * TextScale + PlaceIcon.UL + Offset / 4) * ScaleUI,
...
void AShooterHUD::DrawKills()
{
    ...
    TextItem.SetColor(HUDDark);
    Canvas->DrawItem(
        TextItem, KillsPosX + Offset * ScaleUI + KillsIcon.UL * 1.5f * ScaleUI, KillsPosY + (KillsBg.VL * ScaleUI - SizeY * TextScale * ScaleUI) / 2);
...
void AShooterHUD::DrawHUD()
{
    ...
    if (PlayerOwner && PlayerOwner->IsSplitscreenPlayer(&SSPlayerIndex))
    {
        ...
        if (LP)
        {
            ...
            if ((SSType == ESplitScreenType::TwoPlayer_Horizontal) || (SSType == ESplitScreenType::ThreePlayer_FavorBottom && SSPlayerIndex == 2) ||
    ...
    if (MyPawn && MyPawn->IsAlive() && MyPawn->Health < MyPawn->GetMaxHealth() * MyPawn->GetLowHealthPercentage())
    {
        ...
        Canvas->PopSafeZoneTransform();
        FCanvasTileItem TileItem(
            FVector2D(0, 0), LowHealthOverlayTexture->Resource, FVector2D(Canvas->ClipX, Canvas->ClipY), FLinearColor(1.0f, 0.0f, 0.0f, AlphaValue));
        TileItem.BlendMode = SE_BLEND_Translucent;
        Canvas->DrawItem(TileItem);
        Canvas->ApplySafeZoneTransform();
    }
...
void AShooterHUD::DrawDebugInfoString(const FString& Text, float PosX, float PosY, bool bAlignLeft, bool bAlignTop, const FColor& TextColor)
{
    ...
    FCanvasTileItem TileItem(
        FVector2D(X, Y), FVector2D((SizeX + BoxPadding * SCALE_Y) * ScaleUI, (SizeY * SCALE_Y + BoxPadding * SCALE_Y) * ScaleUI), DrawColor);
    TileItem.BlendMode = SE_BLEND_Translucent;
    Canvas->DrawItem(TileItem);
    ...
    if (PCOwner)
    {
            ...
            (Pawn->IsTargeting() || (!Pawn->IsTargeting() && !Pawn->GetWeapon()->bHideCrosshairWhileNotAiming)))
        {
            ...
            else
            {
                ...
                Canvas->DrawIcon(*CurrentCrosshair[EShooterCrosshairDirection::Right], CenterX + CrossSpread * ScaleUI,
                ...
                Canvas->DrawIcon(*CurrentCrosshair[EShooterCrosshairDirection::Bottom],
                    CenterX - (*CurrentCrosshair[EShooterCrosshairDirection::Bottom]).UL * ScaleUI / 2.0f, CenterY + CrossSpread * ScaleUI, ScaleUI);
            }
            ...
            if (CurrentTime - LastEnemyHitTime >= 0 && CurrentTime - LastEnemyHitTime <= LastEnemyHitDisplayTime)
            {
                ...
                Canvas->DrawIcon(
                    HitNotifyCrosshair, CenterX - HitNotifyCrosshair.UL * ScaleUI / 2.0f, CenterY - HitNotifyCrosshair.VL * ScaleUI / 2.0f, ScaleUI);
            }
        }
    }
}
...
void AShooterHUD::DrawDeathMessages()
{
    ...
    for (int32 i = DeathMessages.Num() - 1; i >= 0; i--)
    {
        ...
        if (Message.DamageType.IsValid())
        {
            ...
            Canvas->DrawIcon(Message.DamageType->KillIcon, CurrentX + (OffsetX / 4.0f) * ScaleUI,
...
void AShooterHUD::ShowDeathMessage(
    class AShooterPlayerState* KillerPlayerState, class AShooterPlayerState* VictimPlayerState, const UDamageType* KillerDamageType)
{
...
void AShooterHUD::NotifyWeaponHit(float DamageTaken, struct FDamageEvent const& DamageEvent, class APawn* PawnInstigator)
{
    ...
    if (MyPawn)
    {
        ...
        FVector Dirs2[8] = {
            FVector(0, -1, 0) /*left*/, FVector(1, -1, 0) /*front left*/, FVector(1, 0, 0) /*front*/, FVector(1, 1, 0) /*front right*/,
            FVector(0, 1, 0) /*right*/, FVector(-1, 1, 0) /*back right*/, FVector(-1, 0, 0), /*back*/
...
void AShooterHUD::DrawHitIndicator()
{
    ...
    if (CurrentTime - LastHitTime <= HitNotifyDisplayTime)
    {
        ...
        for (uint8 i = 0; i < 8; i++)
        {
            ...
            Canvas->DrawIcon(HitNotifyIcon[i], StartX + (HitNotifyIcon[i].U - HitNotifyTexture->GetSizeX() / 2 + Offsets[i].X) * ScaleUI,
                StartY + (HitNotifyIcon[i].V - HitNotifyTexture->GetSizeY() / 2 + Offsets[i].Y) * ScaleUI, ScaleUI);
        }
    }
}
    ...
    if (bIsScoreBoardVisible)
    {
        SAssignNew(ScoreboardWidgetOverlay, SOverlay) +
            SOverlay::Slot()
                ...
                .Padding(
                    FMargin(50))[SAssignNew(ScoreboardWidget, SShooterScoreboardWidget).PCOwner(MakeWeakObjectPtr(PlayerOwner)).MatchState(GetMatchState())];
...
bool AShooterHUD::TryCreateChatWidget()
{
    ...
    else
    {
        ...
        if (ChatWidget.IsValid() == false)
        {
            ...
            GEngine->GameViewport->AddViewportWidgetContent(SAssignNew(ChatWidget, SChatWidget, WorldContext) /*.bKeepVisible(true)*/
            );
        }
    }
    return bCreated;
...
float AShooterHUD::DrawRecentlyKilledPlayer()
{
    ...
    if (MatchState == EShooterMatchState::Playing)
    {
        ...
        if (CurrentTime - LastKillTime >= 0 && CurrentTime - LastKillTime <= KillFadeOutTime)
        {
            ...
            Canvas->DrawItem(TextItem,
                Canvas->OrgX + Canvas->ClipX / 2 - (KilledIcon.UL * ScaleUI + SizeX * TextScale * ScaleUI) / 2.0f + KilledIcon.UL * ScaleUI,
