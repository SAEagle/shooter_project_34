...
void AShooterExplosionEffect::BeginPlay()
{
    ...
    if (NiagaraExplosionFX)
    {
        UNiagaraComponent* effect = UNiagaraFunctionLibrary::SpawnSystemAtLocation(
            this, NiagaraExplosionFX, GetActorLocation(), GetActorRotation(), FVector(1), true, true, ENCPoolMethod::AutoRelease, true);
    }
    ...
    if (Decal.DecalMaterial)
    {
        ...
        UGameplayStatics::SpawnDecalAttached(Decal.DecalMaterial, FVector(Decal.DecalSize, Decal.DecalSize, 1.0f), SurfaceHit.Component.Get(),
            SurfaceHit.BoneName, SurfaceHit.ImpactPoint, RandomDecalRotation, EAttachLocation::KeepWorldPosition, Decal.LifeSpan);
    }
}
